...
char* bserver::sign_msg(BIGNUM *msg_to_sign) {
    //rand r, y=(r^ex)^d mod N, send yr^-1 mod N
...
// Measure time
    ...
    //r = r^e mod N
    BIGNUM *r_exp = BN_new();
    BN_mod_exp(r_exp, r, e, N, ctx);

    //y = (r*x)^d mod N
    BIGNUM *y = BN_new();
    BIGNUM *tmp = BN_new();
    BN_mul(y, r_exp, msg_to_sign, ctx);
    BN_mod_exp(y, y, d, N, ctx);

    // to send
    //tmp = r^-1 mod N
    //y = y*tmp mod N
    BN_mod_inverse(tmp, r, N, ctx);
    ...
    sign_times= diff;
    FILE *file;
    file = fopen("signing2048" , "a");
    fprintf(file, std::to_string(sign_times).c_str());
    fprintf(file, "\n");
